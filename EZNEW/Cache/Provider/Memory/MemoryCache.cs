...
using EZNEW.Cache.Provider.Memory.Abstractions.Internal;
...
namespace EZNEW.Cache.Provider.Memory
{
    ...
    {
        private readonly ConcurrentDictionary<object, MemoryCacheEntry> _entries;
        ...
        public MemoryCache(IOptions<MemoryCacheOptions> optionsAccessor)
        {
            ...
            _entries = new ConcurrentDictionary<object, MemoryCacheEntry>();
            _setEntry = SetEntry;
        ...
        private ICollection<KeyValuePair<object, MemoryCacheEntry>> EntriesCollection => _entries;
        ...
        private void SetEntry(MemoryCacheEntry entry)
        {
            ...
            if (_entries.TryGetValue(entry.Key, out MemoryCacheEntry priorEntry))
            {
        ...
        /// <inheritdoc />
        public bool TryGetEntry(object key, out ICacheEntry result)
        {
            ValidateCacheKey(key);
            ...
            if (_entries.TryGetValue(key, out MemoryCacheEntry entry))
            {
        ...
        public List<string> GetAllKeys()
        {
            ...
            return _entries.Keys.Select(c => c.ToString()).ToList();
        }
        ...
        public void Remove(object key)
        {
            ...
            CheckDisposed();
            if (_entries.TryRemove(key, out MemoryCacheEntry entry))
            {
                if (_options.SizeLimit.HasValue)
                {
                    Interlocked.Add(ref _cacheSize, -entry.Size.Value);
        ...
        {
            if (EntriesCollection.Remove(new KeyValuePair<object, MemoryCacheEntry>(entry.Key, entry)))
            {
                if (_options.SizeLimit.HasValue)
                {
                    Interlocked.Add(ref _cacheSize, -entry.Size.Value);
        ...
        private bool UpdateCacheSizeExceedsCapacity(MemoryCacheEntry entry)
        {
            if (!_options.SizeLimit.HasValue)
            {
                return false;
        ...
        private void Compact(long removalSizeTarget, Func<MemoryCacheEntry, long> computeEntrySize)
        {
            var entriesToRemove = new List<MemoryCacheEntry>();
            var lowPriEntries = new List<MemoryCacheEntry>();
            var normalPriEntries = new List<MemoryCacheEntry>();
            var highPriEntries = new List<MemoryCacheEntry>();
            long removedSize = 0;
        ...
        /// ?. Items with the soonest sliding expiration.
        /// ?. Larger objects - estimated by object graph size, inaccurate.
        private void ExpirePriorityBucket(ref long removedSize, long removalSizeTarget, Func<MemoryCacheEntry, long> computeEntrySize, List<MemoryCacheEntry> entriesToRemove, List<MemoryCacheEntry> priorityEntries)
        {
