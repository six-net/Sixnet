...
namespace EZNEW.Development.Domain.Repository.Warehouse
{
    ...
    public class DataPackage<TEntity> where TEntity : BaseEntity<TEntity>, new()
    {
        ...
        public void Modify(IModification modify)
        {
            if (modify == null)
            {
                return;
        ...
        /// <param name="data">Data</param>
        /// <param name="modify">Modify expression</param>
        void ModifyData(TEntity data, IModification modify)
        {
            if (data == null || modify == null)
            {
                return;
            }
            var modifyValues = modify.GetModificationValues() ?? new Dictionary<string, IModificationValue>(0);
            ...
            foreach (var mv in modifyValues)
            {
                ...
                var newValue = modifyValue.GetModifiedValue(nowValue);
