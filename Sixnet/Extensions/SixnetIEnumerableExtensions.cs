...
namespace System.Collections.Generic
{
    ...
    /// </summary>
    public static class SixnetIEnumerableExtensions
    {
        ...
        /// <param name="name">Key name</param>
        /// <returns>Return the value</returns>
        public static SixnetFieldsAssignment GetFieldsAssignment(this IDictionary<string, dynamic> dict)
        {
            var fieldsAssignment = new SixnetFieldsAssignment();
        ...
        public static DataTable ToDataTable<T>(this IEnumerable<T> datas, SixnetDatabaseType? databaseType = null)
        {
            ...
            table.TableName = entityConfig == null
                ? dataType.Name
                : SixnetDataManager.GetDefaultTableName(SixnetDataCommandExecutionContext.Create(null, null), entityConfig).FullName;
        ...
        /// <returns></returns>
        public static List<SixnetNameValue<TValue>> GetNameValues<TModel, TValue>(this IEnumerable<TModel> datas, Func<TModel, string> nameSelector, Func<TModel, TValue> valueSelector)
        {
            if (datas.IsNullOrEmpty())
            {
                return new List<SixnetNameValue<TValue>>(0);
            }
            SixnetDirectThrower.ThrowArgNullIf(nameSelector == null, nameof(nameSelector));
            SixnetDirectThrower.ThrowArgNullIf(valueSelector == null, nameof(valueSelector));

            return datas.Select(c => new SixnetNameValue<TValue>()
            {
                Name = nameSelector(c),
