...
{
    public static class SixnetExpressionExtensions
    {
        /// <summary>
        ...
        /// <param name="expression"></param>
        /// <returns></returns>
        internal static SixnetFieldsAssignment GetFieldsAssignment(this Expression expression)
        {
        ...
        internal static ISixnetField GetDataField(this Expression expression, string formatterName = "")
        {
            return SixnetExpressionHelper.GetDataField(expression, string.IsNullOrWhiteSpace(formatterName)
                ? null : SixnetFieldFormatSetting.Create(formatterName));
        }
    }
}
