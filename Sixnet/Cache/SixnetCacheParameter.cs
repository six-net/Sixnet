...
namespace Sixnet.Cache
{
    ...
    /// </summary>
    public abstract class SixnetCacheParameter<TResponse> : ISixnetCacheParameter where TResponse : SixnetCacheResult, new()
    {
        /// <summary>
        /// Gets or sets the cache object
        /// </summary>
        public SixnetCacheObject CacheObject { get; set; }

        /// <summary>
        /// Gets or sets the command flags
        /// </summary>
        public SixnetCacheCommandFlags CommandFlags { get; set; } = SixnetCacheCommandFlags.None;

        /// <summary>
        /// Gets or sets the cache structure pattern
        /// </summary>
        public SixnetCacheStructurePattern StructurePattern { get; set; } = SixnetCacheStructurePattern.Distribute;
        ...
        /// <returns>Return cache result</returns>
        internal async Task<TResponse> ExecuteAsync(SixnetCacheServer server)
        {
        ...
        /// <param name="server">Cache server</param>
        /// <returns>Return cache result</returns>
        internal TResponse Execute(SixnetCacheServer server)
        {
            if (server == null)
            {
        ...
        protected abstract Task<TResponse> ExecuteCacheOperationAsync(ISixnetCacheProvider cacheProvider, SixnetCacheServer server);
        ...
        /// <param name="server">Cache server</param>
        /// <returns>Return cache operation response</returns>
        protected abstract TResponse ExecuteCacheOperation(ISixnetCacheProvider cacheProvider, SixnetCacheServer server);

        /// <summary>
        /// Get cache server
        /// </summary>
        /// <returns>Return cache server</returns>
        protected virtual SixnetCacheServer GetCacheServer()
        {
