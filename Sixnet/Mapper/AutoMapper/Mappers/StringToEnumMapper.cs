...
public sealed class StringToEnumMapper : IObjectMapper
{
        ...
        MemberMap memberMap, Expression sourceExpression, Expression destExpression)
    {
        ...
        var enumParse = Expression.Call(ParseMethod.MakeGenericMethod(destinationType), sourceExpression, ignoreCase);
        var enumMember = CheckEnumMember(sourceExpression, destinationType, enumParse, EqualsMethod);
        return Expression.Condition(Expression.Equal(Expression.Property(sourceExpression, Length), ExpressionBuilder.Zero), configuration.Default(destinationType), enumMember);
    }
