...
namespace Sixnet.MQ.Internal
{
    ...
    /// Internal queue
    /// </summary>
    public class SixnetInternalQueue
    {
        #region Fields
        ...
        private SixnetInternalQueue(string name)
        {
        ...
        /// <returns></returns>
        public static SixnetInternalQueue Create(string name)
        {
            return new SixnetInternalQueue(name);
        }
        ...
        public void Enqueue(IEnumerable<SixnetQueueMessage> messages)
        {
            ...
            foreach (var item in messages)
            {
                try
                {
                    ...
                    {
                        SixnetLogger.LogProvider?.WriteLog(typeof(SixnetInternalQueue).FullName, LogLevel.Information, SixnetLogEvents.Framework.InternalQueueEnqueueError
                            , null, $"Internal queue write failure: {SixnetJsonSerializer.Serialize(item)}");
                    }
                }
                catch (Exception ex)
                {
                    SixnetLogger.LogProvider?.WriteLog(typeof(SixnetInternalQueue).FullName, LogLevel.Error, SixnetLogEvents.Framework.InternalQueueEnqueueError, ex, ex.Message);
                }
            }
        }
            ...
            if (count > 0)
            {
                lock (this)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        SixnetLogger.LogProvider?.WriteLog(typeof(SixnetInternalQueue).FullName, LogLevel.Information, SixnetLogEvents.Framework.InternalQueueStartConsumer
        ...
        async Task ConsumeAction(CancellationToken token)
        {
            ...
            while (!IsStopConsume())
            {
                ...
                {
                    SixnetLogger.LogError<SixnetInternalQueue>(SixnetLogEvents.Framework.InternalQueueConsumeError, ex, ex.Message);
                }
            }
            SixnetLogger.LogProvider?.WriteLog(typeof(SixnetInternalQueue).FullName, LogLevel.Information, SixnetLogEvents.Framework.InternalQueueCloseConsumer
