...
namespace Sixnet.Algorithm.Selection
{
    ...
    /// </summary>
    [Serializable]
    public class SixnetDataSelecter<T>
    {
        readonly SixnetShuffleNet<T> shuffleNet = null;
        readonly List<T> values = null;
        int pollingIndex = -1;

        /// <summary>
        /// Initializes a new instance of the Sixnet.Selection.SixnetDataSelecter class
        /// </summary>
        /// <param name="datas">Datas</param>
        public SixnetDataSelecter(IEnumerable<T> datas)
        {
            values = datas?.ToList() ?? new List<T>(0);
            shuffleNet = new SixnetShuffleNet<T>(values, true, true);
        }
