...
namespace Sixnet.Code
{
    ...
    /// </summary>
    public static class SixnetRandomNumberHelper
    {
        #region Fields
        ...
        static SixnetRandomNumberHelper()
        {
            random = new Random();
        }
        ...
        public static int GetRandomNumberByLength(int maxLength, bool constraintMaxLength = false)
        {
            ...
            var maxValue = maxLength >= 10 ? int.MaxValue : SixnetNumberHelper.GetMaximum(maxLength);
            var minValue = constraintMaxLength ? SixnetNumberHelper.GetMinimum(maxLength) : 0;
