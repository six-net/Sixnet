...
namespace Sixnet.Session
{
    ...
    /// Defines user info
    /// </summary>
    public class SixnetUserInfo : IIdentity
    {
        #region Fields
        ...
        public static SixnetUserInfo GetUserFromPrincipal(ClaimsPrincipal principal)
        {
            if (principal == null)
            {
                return null;
        ...
        /// <returns>Return authentication user</returns>
        public static SixnetUserInfo GetUserFromClaims(IEnumerable<Claim> claims)
        {
            if (claims.IsNullOrEmpty())
            {
                return null;
            }
            var idClaim = claims.FirstOrDefault(c => c.Type == SixnetJwtClaimTypes.Subject);
            idClaim ??= claims.FirstOrDefault(c => c.Type == ClaimTypes.NameIdentifier);

            var nameClaim = claims.FirstOrDefault(c => c.Type == SixnetJwtClaimTypes.Name);
            nameClaim ??= claims.FirstOrDefault(c => c.Type == ClaimTypes.Name);

            var givenNameClaim = claims.FirstOrDefault(c => c.Type == SixnetJwtClaimTypes.GivenName);
            givenNameClaim ??= claims.FirstOrDefault(c => c.Type == ClaimTypes.GivenName);

            var nickNameClaim = claims.FirstOrDefault(c => c.Type == SixnetJwtClaimTypes.NickName);
            nickNameClaim ??= claims.FirstOrDefault(c => c.Type == ClaimTypes.Surname);

            var roleClaim = claims.FirstOrDefault(c => c.Type == SixnetJwtClaimTypes.Role);
        ...
        public virtual List<Claim> GetClaims()
        {
            ...
            {
                new Claim(SixnetJwtClaimTypes.Subject,Id.ToString()),
                new Claim(SixnetJwtClaimTypes.Name,Name??string.Empty),
                new Claim(SixnetJwtClaimTypes.NickName,DisplayName??string.Empty),
                new Claim(SixnetJwtClaimTypes.GivenName,PersonName??string.Empty),
                ...
                new Claim(SixnetJwtClaimTypes.Role,Roles.IsNullOrEmpty()? "": string.Join(',',Roles))
            };
        }
