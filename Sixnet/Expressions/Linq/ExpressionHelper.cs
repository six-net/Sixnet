...
namespace Sixnet.Expressions.Linq
{
    public static class ExpressionHelper
    {
        ...
        /// </summary>
        static readonly ConcurrentDictionary<string, dynamic> PropertyOrFieldAccessFunctions = new();

        /// <summary>
        /// Expression base type
        /// </summary>
        static readonly Type BaseExpressType = typeof(Expression);
        ...
        {
            var queryable = GetQueryable(conditionExpression, connector) ?? SixnetQueryable.Create<T>();
            if (queryable != null)
            {
        ...
        static ISixnetQueryable GetQueryableCore(Dictionary<string, int> parameterIndexes, CriterionConnector connector, Expression conditionExpression)
        {
            ...
            var groupQueryable = SixnetQueryable.Create();
        ...
        internal static Criterion GetExpressionCriterion(Dictionary<string, int> parameterIndexes, CriterionConnector connector, Expression conditionExpression)
        {
            if (conditionExpression is BinaryExpression binaryExpression)
            {
                ...
                ThrowHelper.ThrowFrameworkErrorIf(leftField == null && rightField == null, "Left and right expression is null");

                var criterionOperator = GetCriterionOperator(nodeType);
                if (leftField == null || rightField == null)
                {
                    ThrowHelper.ThrowFrameworkErrorIf(criterionOperator != CriterionOperator.Equal && criterionOperator != CriterionOperator.NotEqual, $"Not support for {nodeType}");
        ...
        /// <returns></returns>
        internal static ICondition GetMethodCriterion(Dictionary<string, int> parameterIndexes, CriterionConnector connector, Expression conditionExpression)
        {
            var methodCallExpression = conditionExpression as MethodCallExpression;
            Expression fieldExpression = null;
            Expression valueExpression = null;
            ICondition criterion = null;
            ...
            if (methodCallExpression.Method.IsStatic) // Static method
            {
                ...
                else if (methodType == typeof(SixnetQueryable))
                {
                    fieldExpression = methodCallExpression.Arguments[0];
            ...
            ThrowHelper.ThrowFrameworkErrorIf(criterion == null, $"Not support {methodCallExpression.Method.Name}");
        ...
        /// <returns>Return a criterion</returns>
        internal static ICondition GetIEnumerableMethodCriterion(Dictionary<string, int> parameterIndexes, string methodName, CriterionConnector connector, Expression field, Expression collectionValue)
        {
            ICondition criterion = null;
        ...
        internal static ICondition GetStringMethodCriterion(Dictionary<string, int> parameterIndexes, string methodName, CriterionConnector connector, Expression field, Expression stringValue)
        {
            var leftField = GetDataField(field, parameterIndexes);
            if (leftField == null)
            {
            ...
            switch (methodName)
            {
                ...
                case "EndsWith":
                    ...
                    break;
                case "IsNullOrEmpty":
                    var nullOrEmtpyQueryable = SixnetQueryable.Create();
                ...
                case "IsNullOrWhiteSpace":
                    var nullOrWhiteSpaceQueryable = SixnetQueryable.Create();
                    ...
                    copyField.FormatOption = FieldFormatOption.Create(FieldFormatterNames.TRIM);
        ...
        /// <param name="field">Expression</param>
        /// <returns>criterion</returns>
        internal static ICondition GetSubqueryMethodCriterion(Dictionary<string, int> parameterIndexes, string methodName, CriterionConnector connector, Expression field, Expression subquery)
        {
            var leftField = GetDataField(field, parameterIndexes);
            if (leftField == null)
            {
        ...
        /// <returns></returns>
        internal static ICondition GetSixnetQueryableExtendMethodCriterion(Dictionary<string, int> parameterIndexes, string methodName, CriterionConnector connector, Expression field)
        {
            var leftField = GetDataField(field, parameterIndexes);
            if (leftField == null)
            {
        ...
        /// <returns></returns>
        internal static Criterion CreateCriterion(CriterionConnector connector, CriterionOperator criterionOperator, IDataField left, IDataField right)
        {
        ...
        /// <returns></returns>
        public static IDataField GetDataField(Expression fieldExpression)
        {
            ThrowHelper.ThrowArgNullIf(fieldExpression == null, $"{nameof(fieldExpression)} is null");
        ...
        /// <param name="fieldExpression">Field expression</param>
        /// <returns></returns>
        public static IDataField GetDataField(Expression fieldExpression, Dictionary<string, int> parameterIndexes)
        {
            var childExpression = fieldExpression;
            ThrowHelper.ThrowArgNullIf(childExpression == null, $"Not support expression:{fieldExpression?.GetType()}");
            IDataField criterionField = null;
            FieldFormatOption fieldFormatOption = null;
            var isEnd = false;
            ...
            do
            {
                ...
                switch (childExpression.NodeType)
                {
                    case ExpressionType.MemberAccess:
                        ...
                        if (memberExpression.Expression.NodeType == ExpressionType.Parameter)
                        {
                            ...
                            var propertyField = PropertyField.Create(propertyName, modelType, modelTypeIndex);
                ...
                {
                    // field format
                    var newFieldFormatOption = GetFieldFormatOption(childExpression, parameterIndexes);
                    if (newFieldFormatOption != null)
                    {
                        newFieldFormatOption.ChildFormatOption = fieldFormatOption;
                        fieldFormatOption = newFieldFormatOption;
                    }
            ...
            {
                criterionField.FormatOption = fieldFormatOption;
            }
        ...
        /// <param name="fieldExpression">Field expression</param>
        /// <returns></returns>
        public static IDataField GetOutputDataField(ISixnetQueryable queryable, Expression fieldExpression)
        {
            #region Type index
            ...
            do
            {
                ...
                switch (childExpression.NodeType)
                {
                    case ExpressionType.MemberAccess:
                        ...
                        if (memberExpression.Expression.NodeType == ExpressionType.Parameter)
                        {
                            ...
                            var modelType = parameterExp.Type;
                            var modelTypeIndex = 0;
                            typeIndexes?.TryGetValue(modelType.FullName, out modelTypeIndex);
                            var propertyField = PropertyField.Create(propertyName, modelType, modelTypeIndex);
                            propertyField.DataType = memberExpression.Type;
                            criterionField = propertyField;
                            isEnd = true;
                ...
                {
                    // field format
                    var newFieldFormatOption = GetFieldFormatOption(childExpression, null);
                    if (newFieldFormatOption != null)
                    {
                        newFieldFormatOption.ChildFormatOption = fieldFormatOption;
                        fieldFormatOption = newFieldFormatOption;
        ...
        /// <summary>
        /// Get field format option
        /// </summary>
        /// <param name="formatExpression">Format expression</param>
        /// <returns></returns>
        static FieldFormatOption GetFieldFormatOption(Expression formatExpression, Dictionary<string, int> parameterIndexes)
        {
            FieldFormatOption fieldFormatOption = null;
            if (formatExpression != null)
            {
                switch (formatExpression.NodeType)
                {
                    case ExpressionType.MemberAccess:
                        ...
                        // char length
                        if (memberExpression.Member.Name == nameof(string.Length) && memberExpression.Expression.NodeType == ExpressionType.MemberAccess && memberExpression.Expression.Type == typeof(string))
                        {
                            fieldFormatOption = FieldFormatOption.Create(FieldFormatterNames.CHARLENGTH);
                        }
                        break;
                    case ExpressionType.Call:
                        var methodCallExpression = formatExpression as MethodCallExpression;
                        if(methodCallExpression.Object!=null && methodCallExpression.Method.DeclaringType == typeof(string))
                        {
                            if (methodCallExpression.Method.Name == nameof(string.Trim)) // trim
                            {
                                fieldFormatOption = FieldFormatOption.Create(FieldFormatterNames.TRIM);
                            }
                        }
                        else if (methodCallExpression.Object == null && methodCallExpression.Method.IsStatic && methodCallExpression.Method.DeclaringType == typeof(SixnetQueryable))
                        {
                            if (methodCallExpression.Method.Name == nameof(SixnetQueryable.Max)) // max
                            {
                                fieldFormatOption = FieldFormatOption.Create(FieldFormatterNames.MAX);
                            }
                            else if (methodCallExpression.Method.Name == nameof(SixnetQueryable.Min)) // min
                            {
                                fieldFormatOption = FieldFormatOption.Create(FieldFormatterNames.MIN);
                            }
                            else if (methodCallExpression.Method.Name == nameof(SixnetQueryable.Avg)) // avg
                            {
                                fieldFormatOption = FieldFormatOption.Create(FieldFormatterNames.AVG);
                            }
                            else if (methodCallExpression.Method.Name == nameof(SixnetQueryable.Count)) // count
                            {
                                fieldFormatOption = FieldFormatOption.Create(FieldFormatterNames.COUNT);
                            }
                            else if (methodCallExpression.Method.Name == nameof(SixnetQueryable.Sum)) // sum
                            {
                                fieldFormatOption = FieldFormatOption.Create(FieldFormatterNames.SUM);
                            }
                            else if (methodCallExpression.Method.Name == nameof(SixnetQueryable.JsonValue)) // json value
                            {
                                fieldFormatOption = FieldFormatOption.Create(FieldFormatterNames.JSON_VALUE, GetDataField(methodCallExpression.Arguments[1], parameterIndexes));
                            }
                            else if (methodCallExpression.Method.Name == nameof(SixnetQueryable.JsonObject)) // json object
                            {
                                fieldFormatOption = FieldFormatOption.Create(FieldFormatterNames.JSON_OBJECT, GetDataField(methodCallExpression.Arguments[1], parameterIndexes));
                            }
                        }
                        break;
                    case ExpressionType.And:
                        var andExpression = formatExpression as BinaryExpression;
                        fieldFormatOption = FieldFormatOption.Create(FieldFormatterNames.AND, GetDataField(andExpression.Right));
                        break;
                    case ExpressionType.Or:
                        var orExpression = formatExpression as BinaryExpression;
                        fieldFormatOption = FieldFormatOption.Create(FieldFormatterNames.OR, GetDataField(orExpression.Right));
                        break;
                    case ExpressionType.ExclusiveOr:
                        var exclusiveOrExpression = formatExpression as BinaryExpression;
                        fieldFormatOption = FieldFormatOption.Create(FieldFormatterNames.XOR, GetDataField(exclusiveOrExpression.Right));
                        break;
                    case ExpressionType.Not:
                        fieldFormatOption = FieldFormatOption.Create(FieldFormatterNames.NOT);
                        break;
                    case ExpressionType.Add:
                        var addExpression = formatExpression as BinaryExpression;
                        var parameterField = GetDataField(addExpression.Right);
                        var fieldDataType = parameterField.GetFieldDataType();
                        fieldFormatOption = FieldFormatOption.Create((fieldDataType == StringType || fieldDataType == CharType) ? FieldFormatterNames.STRING_CONCAT : FieldFormatterNames.ADD, parameterField);
                        break;
                    case ExpressionType.Subtract:
                        var subtractExpression = formatExpression as BinaryExpression;
                        fieldFormatOption = FieldFormatOption.Create(FieldFormatterNames.SUBTRACT, GetDataField(subtractExpression.Right));
                        break;
                    case ExpressionType.Multiply:
                        var multiplyExpression = formatExpression as BinaryExpression;
                        fieldFormatOption = FieldFormatOption.Create(FieldFormatterNames.MULTIPLY, GetDataField(multiplyExpression.Right));
                        break;
                    case ExpressionType.Divide:
                        var divideExpression = formatExpression as BinaryExpression;
                        fieldFormatOption = FieldFormatOption.Create(FieldFormatterNames.DIVIDE, GetDataField(divideExpression.Right));
                        break;
                    case ExpressionType.Modulo:
                        var moduloExpression = formatExpression as BinaryExpression;
                        fieldFormatOption = FieldFormatOption.Create(FieldFormatterNames.MODULO, GetDataField(moduloExpression.Right));
                        break;
                    case ExpressionType.RightShift:
                        var rightShiftExpression = formatExpression as BinaryExpression;
                        fieldFormatOption = FieldFormatOption.Create(FieldFormatterNames.RIGHT_SHIFT, GetDataField(rightShiftExpression.Right));
                        break;
                    case ExpressionType.LeftShift:
                        var leftShiftExpression = formatExpression as BinaryExpression;
                        fieldFormatOption = FieldFormatOption.Create(FieldFormatterNames.LEFT_SHIFT, GetDataField(leftShiftExpression.Right));
                        break;
                }
            }

            return fieldFormatOption;
        }
        ...
        {
            ThrowHelper.ThrowArgNullIf(updateExpression == null, nameof(updateExpression));
            ThrowHelper.ThrowNotSupportIf(updateExpression.NodeType != ExpressionType.Lambda, updateExpression.NodeType.ToString());
        ...
        static void GenerateFieldsAssignmentCore(FieldsAssignment fieldsAssignment, Dictionary<string, int> parameterIndexes, Expression updateExpression)
        {
            ...
            ThrowHelper.ThrowNotSupportIf(binaryExp == null, binaryExp.NodeType.ToString());
