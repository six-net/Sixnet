...
{
    public static class SixnetConditionExtensions
    {
        #region Clone
        ...
        {
            if (originalCondition is SixnetCriterion criterion)
            {
        ...
        static ISixnetQueryable AppendEntityIdentityCore<TEntity>(IEnumerable<TEntity> entities, ISixnetQueryable originalQueryable = null, bool exclude = false) where TEntity : class, ISixnetEntity<TEntity>
        {
            ...
            foreach (var entity in entities)
            {
                ...
                else
                {
                    ...
                    {
                        entityQueryable = entityQueryable.Where(SixnetCriterion.Create(exclude ? SixnetCriterionOperator.NotEqual : SixnetCriterionOperator.Equal, SixnetDataField.Create(key, entityType), SixnetConstantField.Create(entity.GetValue(key))));
                    }
                    entityQueryable.Connector = SixnetCriterionConnector.Or;
            ...
            if (isSinglePrimaryKey)
            {
                ...
                originalQueryable = exclude
                    ? primaryKeyValueCount > 1
                        ? originalQueryable.Where(SixnetCriterion.Create(SixnetCriterionOperator.NotIn, SixnetDataField.Create(primaryKeys.ElementAt(0), entityType), SixnetConstantField.Create(primaryKeyValues)))
                        : originalQueryable.Where(SixnetCriterion.Create(SixnetCriterionOperator.NotEqual, SixnetDataField.Create(primaryKeys.ElementAt(0), entityType), SixnetConstantField.Create(primaryKeyValues[0])))
                    : primaryKeyValueCount > 1
                        ? originalQueryable.Where(SixnetCriterion.Create(SixnetCriterionOperator.In, SixnetDataField.Create(primaryKeys.ElementAt(0), entityType), SixnetConstantField.Create(primaryKeyValues)))
                        : originalQueryable.Where(SixnetCriterion.Create(SixnetCriterionOperator.Equal, SixnetDataField.Create(primaryKeys.ElementAt(0), entityType), SixnetConstantField.Create(primaryKeyValues[0])));
            }
            else
            {
