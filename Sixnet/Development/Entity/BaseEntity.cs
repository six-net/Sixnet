...
namespace Sixnet.Development.Entity
{
    ...
    /// Base entity
    /// </summary>
    public abstract class BaseEntity<T> : IEntity<T> where T : class, IEntity<T>
    {
        #region Fields
        ...
        {
            var verifyResults = ValidationManager.Validate(this, ValidationConstants.UseCaseNames.Domain);
            var errorMessages = verifyResults.GetErrorMessages();
            ThrowHelper.ThrowFrameworkErrorIf(!errorMessages.IsNullOrEmpty(), JsonSerializer.Serialize(errorMessages));
            return true;
        }
        ...
        {
            var primaryKeys = EntityManager.GetPrimaryKeyNames<T>();
            ...
            {
                var field = EntityManager.GetField(typeof(T), pk);
                if (field != null)
                {
        ...
        {
            var valueProvider = EntityManager.GetField(entityType, propertyName)?.ValueProvider;
        ...
        {
            var primaryKeys = EntityManager.GetPrimaryKeyNames(typeof(T));
        ...
        public Dictionary<string, dynamic> GetAllValues()
        {
            ...
            ThrowHelper.ThrowFrameworkErrorIf(entityConfig == null, $"{entityType.FullName}'s configuration is null");

            var allValues = new Dictionary<string, dynamic>(entityConfig.AllFields.Count);
            foreach (var field in entityConfig.AllFields)
            {
                ThrowHelper.ThrowFrameworkErrorIf(field.Value?.ValueProvider == null, $"{entityType.FullName} => {field.Key}'s value provider is null");
        ...
        public FieldsAssignment GetFieldsAssignment(Dictionary<string, dynamic> oldValues = null)
        {
            ...
            {
                var field = EntityManager.GetField(entityType, valueItem.Key);
                ...
                else
                {
                    ...
                    bool isUpdated = isValueType
                        ...
                        : field.DataType.IsSerializable
                                        ? BinarySerializer.SerializeObjectToString(oldValue) != BinarySerializer.SerializeObjectToString(newValue)
                                        : oldValue != newValue;
                    if (isUpdated)
                    {
                        fieldsAssignment.SetNewValue(valueItem.Key, valueItem.Value);
