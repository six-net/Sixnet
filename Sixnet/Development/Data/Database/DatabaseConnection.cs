...
namespace Sixnet.Development.Data.Database
{
    ...
    public class DatabaseConnection
    {
        ...
        /// </summary>
        private SixnetLock? ConnectionLock { get; set; }
        ...
        private DatabaseConnection(DatabaseServer databaseServer, bool useTransaction, DataIsolationLevel? isolationLevel = null, SixnetLock? connLock = null)
        {
            if (databaseServer == null)
            {
                throw new ArgumentNullException(nameof(databaseServer));
            }
            DatabaseProvider = DataManager.GetDatabaseProvider(databaseServer.ServerType);
        ...
        IDbTransaction GetDbTransaction()
        {
            var defaultIsolationLevel = DataIsolationLevel.HasValue
                ? DataManager.GetSystemIsolationLevel(DataIsolationLevel.Value)
                : DataManager.GetSystemIsolationLevel(DataManager.GetDataIsolationLevel(DatabaseServer.ServerType));
        ...
        public static DatabaseConnection Create(DatabaseServer server, bool useTransaction, DataIsolationLevel? isolationLevel = null)
        {
            ...
            var connLock = LockManager.GetCreateDatabaseConnectionLock(server);
            if (connLock != null)
            {
